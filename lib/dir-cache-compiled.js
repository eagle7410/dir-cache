...
class Scan extends Frame {
	...
	isCorrectExt(file) {
		for (let i = 0; i < this.exts.length; i++) if (~file.indexOf(this.exts[i])) return true;
	...
	_dirFiles(dir, endDir, add, res) {
		...
		fs.readdir(dir, (e, arFile) => {
			...
			arFile.filter(file => {
				...
				if (that.isCorrectExt(file)) return true;
...
//# sourceMappingURL=dir-cache-compiled.js.map