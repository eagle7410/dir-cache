...
class Cache extends  Frame {
	constructor (options, cb) {
		...
		let dirs = [];
		utils.obj.for(options.cache, (k, v) => {

			if (that.stop) {
				return;
			}

			if (!utils.obj.isObj(v)) {
				return that.stop = `value for ${k} not object`;
			}

			if (!utils.type.isSet(v.dirs) || utils.obj.isEmpty(v.dirs)) {
				return that.stop = `value for ${k} not have or empty dirs`;
			}

			options.cache[k].dirs = Array.isArray(v.dirs) ? v.dirs : [v.dirs];

			dirs = dirs.concat(v.dirs);
		...
		log.info(`Be async call for check exists folders [${dirs}]`);

		async.map(dirs, (dir, endDir) => {
...
exports.files = (options, cb) => new Cache (options, cb);
//
